PROJECT_NAME := containerregistry Package

SHELL            := /bin/bash
PACK             := containerregistry
ORG              := rhysmdnz
PROJECT          := github.com/${ORG}/pulumi-${PACK}
NODE_MODULE_NAME := @rhysmdnz/${PACK}
TF_NAME          := ${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/rhysmdnz/pulumi-containerregistry,${REPOSITORY},g' provider/go.mod; \
...
build_python:: install_plugins tfgen # build the python sdk
	...
	cd sdk/python/ && \
        ...
        rm ./bin/setup.py.bak && \
        cd ./bin && python3 setup.py build sdist && python3 setup.py bdist_wheel
